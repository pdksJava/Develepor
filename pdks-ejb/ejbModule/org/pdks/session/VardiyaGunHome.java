...
public class VardiyaGunHome extends EntityHome<VardiyaPlan> implements Serializable {
	...
	private boolean fileImport = Boolean.FALSE, topluGuncelleme, yasalFazlaCalismaAsanSaat = Boolean.FALSE, fazlaMesaiTalepVar = Boolean.FALSE, fazlaMesaiOde, fazlaMesaiIzinKullan, modelGoster = Boolean.FALSE, gebeGoster = Boolean.FALSE;
	...
	public String mesaiSureHesapla() {
		...
		if (bitisZamani.after(baslangicZamani)) {
			...
			double mesaiSuresi = PdksUtil.setSureDoubleTypeRounded(toplamSure - farkSure, seciliVardiyaGun.getYarimYuvarla());
	...
	public String mesaiIptal() {
		...
		OzelAciklama ozelIptalAciklama = null;
		String aciklama = PdksUtil.hasStringValue(islemFazlaMesaiTalep.getAciklamaIptalMudur()) ? islemFazlaMesaiTalep.getAciklamaIptalMudur().trim() : null;
		if (aciklama != null) {
			ozelIptalAciklama = new OzelAciklama(aciklama);
			saveOrUpdate(ozelIptalAciklama);
		}
		islemFazlaMesaiTalep.setOzelIptalAciklama(ozelIptalAciklama);
		...
		if (islemVardiya != null) {
			...
			if (devam) {
				...
				if (list.isEmpty()) {
					...
					try {
						OzelAciklama ozelAciklama = fazlaMesaiTalep.getOzelAciklama();
						if (topluGuncelle == false) {
							String aciklama = PdksUtil.hasStringValue(fazlaMesaiTalep.getAciklamaYonetici()) ? fazlaMesaiTalep.getAciklamaYonetici().trim() : null;
							if (aciklama != null) {
								ozelAciklama = new OzelAciklama(aciklama);
			...
			if (vardiyaGun.getVardiya() != null) {
				...
				if (vardiya.isCalisma() && vardiyaMap != null && vardiyaGun.isAyinGunu()) {
					if (vardiya.getGenel()) {
						...
						if (vardiya.isSutIzniMi())
							ozel = map.containsKey(Vardiya.SUT_IZNI_KEY) && map.get(Vardiya.SUT_IZNI_KEY);
		...
		if (haftaTatilSb.length() == 0) {
			if (!yaz || (ikMesaj && sb.length() > 0)) {
				...
				if (personelDenklestirme != null) {
					...
					if (ap != null)
						ap.setPersonelDenklestirme(personelDenklestirme);
				}
			} else {
	 ...
	 * @return
	 */
	public String aylikPuantajSec(AylikPuantaj ap, String tipi) {
		...
		if (tipi.equals("M")) {
			...
			if (yoneticiOnay2 == null) {
				Personel yoneticiOnay1 = ap.getYonetici();
				if (personel.getAsilYonetici2() != null || yoneticiOnay1 != null) {
					if (personel.getAsilYonetici2() != null)
						yoneticiOnay2 = personel.getAsilYonetici2();
					else if (yoneticiOnay1.getYoneticisi() != null)
						yoneticiOnay2 = yoneticiOnay1.getYoneticisi();
				...
				if (yoneticiOnay2 != null) {
					if (yoneticiOnay2.getId() != null && yoneticiOnay2.getId().equals(getPdksUser().getPersonelId()) == false) {
						if (yoneticiOnay2.isCalisiyor()) {
							...
							if (user != null) {
								onayDurum = user.isDurum();
								if (onayDurum == false)
									PdksUtil.addMessageAvailableWarn(yoneticiOnay2.getAdSoyad() + " kullanıcısı " + user.getUsername() + " aktif değildir!");

							} else
								PdksUtil.addMessageAvailableWarn(yoneticiOnay2.getAdSoyad() + " onaylayacak kullanıcısı yok!");
						}
					}
				} else
					PdksUtil.addMessageAvailableWarn(personel.getAdSoyad() + " fazla mesai onaylacak yöneticisi yok!");
				if (onayDurum == false)
					tipi = "";
			}
		}
		if (tipi.equals("") == false) {
			...
			if (tipi.equalsIgnoreCase("P")) {
				...
				if (denklestirmeAyDurum) {
					...
					if (ap.getPersonelDenklestirme() != null) {
						if (modelList.isEmpty() == false || adminRole || loginUser.isIK())
							ortakIslemler.addObjectList(ap.getPersonelDenklestirme().getCalismaModeliAy(), modelList, null);
					}
				} else
				...
				if (personel.isGebelikSutIzinVar()) {
					...
					personelSutIzniDurum = ap.getPersonelDenklestirme() != null ? ap.getPersonelDenklestirme().getSutIzniPersonelDonemselDurum() : null;
					for (Iterator iterator = list.iterator(); iterator.hasNext();) {
						...
						if (pdd.isGebe()) {
							...
							if (personelGebeDurum == null) {
								...
								personelGebeDurum.setPersonel(ap.getPdksPersonel());
			...
			if (ap.getSablonAylikPuantaj().getSonGun().getTime() < personel.getSonCalismaTarihi().getTime())
			...
			try {
				if (tipi.equals("P")) {
					...
					list = null;
					idList = null;
					ortakIslemler.vardiyaCalismaModeliGuncelle(ap.getVardiyalar(), session);
				}
			} catch (Exception ed) {
				logger.error(ed);
				ed.printStackTrace();
			...
			ap.setGorevYeriSec(Boolean.FALSE);
			...
			if (ap.getPersonelDenklestirme() != null)
				ap.getPersonelDenklestirme().setGuncellendi(Boolean.FALSE);
			vardiyaDbMap = new HashMap<Long, Vardiya>();
			for (VardiyaGun pdksVardiyaGun : ap.getVardiyalar()) {
				...
				if (!gebeSutIzniGuncelle && pdksVardiyaGun.isAyinGunu() && pdksVardiyaGun.getVardiya() != null && AylikPuantaj.getGebelikGuncelle())
					gebeSutIzniGuncelle = pdksVardiyaGun.getIzin() == null && pdksVardiyaGun.getVardiya().getGebelik();
				setVardiyalarToMap(pdksVardiyaGun, vm, ap);
			...
			if (vgIlkAy != null) {
				...
				for (VardiyaGun digerVardiyaGun : digerVardiyaGunList) {
					setVardiyalarToMap(digerVardiyaGun, vm, ap);
				}
			}
			...
			planYetkilendir(ap);
			sablonGuncelle = getPdksUser() == null && denklestirmeAyDurum && personelAylikPuantaj.isKaydet() && ap.isKaydet();
			if (sablonGuncelle) {
			...
			if (sablonGuncelle) {
				...
				if (ap.getVardiyaHaftaList() != null) {
					for (VardiyaHafta pdksVardiyaHafta : ap.getVardiyaHaftaList()) {
	...
	private boolean aylikVardiyaKontrolKaydet(Boolean aylik) throws Exception {
		...
		if (durum || pdGuncellendi) {
			...
			if (vardiyaGuncellendi && !vardiyalarMap.isEmpty())
				durum = vardiyaPlanKontrol(personelAylikPuantaj, personelDenklestirme, null, null, plan, "", false) || ortakIslemler.getParameterKey("calismaPlanKaydetme").equals("1") == false;
			if (durum)
	...
	public void fillSirketDoldur(boolean tesisDoldur) {
		...
		if (denklestirmeAy != null) {
			if (aramaSecenekleri.getDepartmanId() != null) {
				...
				if (ikRole || adminRole)
					sirketler = fazlaMesaiOrtakIslemler.getFazlaMesaiSirketList(adminRole ? aramaSecenekleri.getDepartmanId() : null, denklestirmeAy != null ? aylikPuantajDonem : null, getDenklestirmeDurum(), session);
				else
	...
	public void fillTesisDoldur(boolean bolumDoldur) {
		...
		if (ikRole || adminRole)
			list = fazlaMesaiOrtakIslemler.getFazlaMesaiTesisList(aramaSecenekleri.getSirket(), denklestirmeAy != null ? aylikPuantajDonem : null, getDenklestirmeDurum(), session);
		else
			list = fazlaMesaiOrtakIslemler.getFazlaMesaiMudurTesisList(aramaSecenekleri.getSirket(), denklestirmeAy != null ? aylikPuantajDonem : null, getDenklestirmeDurum(), fazlaMesaiTalepDurum, session);
		if (list != null && sirket != null) {
			if (list.isEmpty() && sirket.isTesisDurumu())
				list = fazlaMesaiOrtakIslemler.getFazlaMesaiTesisList(aramaSecenekleri.getSirket(), denklestirmeAy != null ? aylikPuantajDonem : null, getDenklestirmeDurum(), session);
		...
		if (aylikPuantaj != null) {
			...
			for (Iterator iterator = vardiyalar.iterator(); iterator.hasNext();) {
				...
				if (personelSutIzniDurum != null) {
					if (!vardiya.isSutIzniMi())
						vardiyaSutIzniOzelMap.put(vardiya.getId(), vardiya);
				}
	...
	public String planOnayla() {
		...
		for (AylikPuantaj ap : aylikPuantajList) {
			...
			if (ap.getCalismaModeli() != null) {
				Long id = ap.getCalismaModeli().getId();
			...
			VardiyaPlan pdksVardiyaPlan = ap.getVardiyaPlan();
			boolean asilOnay = false;
			PersonelDenklestirme personelDenklestirme = ap.getPersonelDenklestirme();
			if (onayDurum) {
				mesajYaz = false;
				onayDurum = vardiyaPlanKontrol(ap, personelDenklestirme, null, vardiyaMap, pdksVardiyaPlan, personel.getSicilNo() + " " + personel.getAdSoyad() + " ", false);
				if (!onayliVar)
					onayliVar = onayDurum;
				asilOnay = true;
		...
		if (!devam) {
			...
			suaUyariMesaji(null, puantajList);
					...
					if (vardiya.getDurum()) {
						...
						if (vardiya.getGenel().equals(Boolean.FALSE)) {
							...
							if (map.isEmpty()) {
								...
								map.put(Vardiya.SUT_IZNI_KEY, Boolean.FALSE);
							...
							if (vardiya.isSutIzniMi())
								map.put(Vardiya.SUT_IZNI_KEY, Boolean.TRUE);
							if (vardiya.isIcapVardiyasi())
			...
			try {
				...
				if (personel.getId().equals(girisYapan.getId())) {
					...
					if (manuelGiris.getId() != null && manuelGiris.getId() != null) {
						...
						Calendar cal = Calendar.getInstance();
						Long nedenId = fmt.getMesaiNeden() != null ? fmt.getMesaiNeden().getId() : null;
						...
						if (fmt.getOnayDurumu() != FazlaMesaiTalep.ONAY_DURUM_RED && fmt.getDurum()) {
							boolean flush = false;
							...
							if (cikisHareket == null && (ardisikAdet == null || ardisikAdet < 2)) {
								if (nedenId != null) {
									hareketEkleReturn(guncelleyen, manuelCikis, personelKGS, tarih2, nedenId, aciklama + " çıkış" + (manuel ? " manuel sonradan eklendi." : ""));
									flush = true;
								}
							...
							Long sonuc = fazlaMesaiOrtakIslemler.fazlaMesaiOtomatikHareketSil(fmt.getId(), session);
							if (sonuc != null && sonuc.equals(fmt.getId()))
								sessionFlush();
						}
	...
	private Boolean saveOnay(Session session, AylikPuantaj aylikPuantaj) {
		...
		if (getPdksUser().getLogin() == false) {
			pd = (PersonelDenklestirme) pdksEntityController.getSQLParamByFieldObject(PersonelDenklestirme.TABLE_NAME, PersonelDenklestirme.COLUMN_NAME_ID, pd.getId(), PersonelDenklestirme.class, session);
			aylikPuantaj.setPersonelDenklestirme(pd);
		}
		if (pd.isOnaylandi() == false) {
			pd.setOnaylandi(Boolean.TRUE);
			if (pd.getSutIzniDurum() == null || pd.getSutIzniDurum().equals(Boolean.FALSE))
				pd.setSutIzniDurum(personel.getSutIzni() != null && personel.getSutIzni());
			if (!pd.isKapandi())
	...
	public String mesaiTopluKaydet() {
		...
		if (!puantajMap.isEmpty()) {
			...
			String aciklama = PdksUtil.hasStringValue(asil.getAciklamaYonetici()) ? asil.getAciklamaYonetici().trim() : null;
			OzelAciklama ozelAciklama = aciklama != null ? new OzelAciklama(aciklama) : null;
			for (Iterator iterator = aylikPuantajMesaiTalepList.iterator(); iterator.hasNext();) {
				AylikPuantaj aylikPuantaj = (AylikPuantaj) iterator.next();
				VardiyaGun vardiyaGunPer = aylikPuantaj.getVardiyaGun();
		...
		if (puantajList != null) {
			...
			for (Iterator iterator = list.iterator(); iterator.hasNext();) {
				...
				if (pd.isOnaylandi() || cma.isHareketKaydiVardiyaBulsunmu() == false)
					...
					if (ap.getVardiyaPlan() == null) {
						TreeMap<String, VardiyaGun> vardiyaGunMap = new TreeMap<String, VardiyaGun>();
						for (VardiyaGun vg : ap.getVardiyalar())
							vardiyaGunMap.put(vg.getVardiyaKeyStr(), vg);

						ortakIslemler.puantajHaftalikPlanOlustur(false, null, vardiyaGunMap, aylikPuantajDefault, ap);
					}
				}
	...
	public Boolean aylikPuantajOlusturuluyor() {
		...
		try {
			...
			AylikPuantaj personeAylikPuantaj = denklestirmeAy != null ? aylikPuantajDonem : null;
			if (personeAylikPuantaj != null) {
					...
					for (Personel personel : personelList) {
						...
						if (denklestirmeAyDurum) {
							...
							if (kaydet) {
								...
								if (denklestirmeAyDurum) {
									...
									personelDenklestirme.setDenklestirme(fazlaMesaiHesaplaTumPersonel || denklestirme);
						...
						for (Iterator iterator2 = aylikSablonVardiyalar.iterator(); iterator2.hasNext();) {
							...
							if (!pdksVardiyaGun.isCalismayaBaslamadi() && !pdksVardiyaGun.isCalismayiBirakti()) {
								...
								if (pdksVardiyaGun.getId() == null && sablonu != null) {
									yeniKayit = true;
									if (pdksVardiyaGun.isAyinGunu())
										tekrarOku = true;
						...
						try {
							if (gecenAylikPuantaj.getPersonelDenklestirme() != null && !helpPersonel(personel) && userLoginOldu) {
			...
			if (topluFazlaCalismaTalep) {
				...
				for (Iterator iterator = aylikPuantajList.iterator(); iterator.hasNext();) {
					AylikPuantaj ap = (AylikPuantaj) iterator.next();
					personelIdler.add(ap.getPdksPersonel().getId());
				}
				katsayiList.add(PuantajKatSayiTipi.GUN_FMT_DURUM.value());
				HashMap<PuantajKatSayiTipi, TreeMap<String, BigDecimal>> allMap = ortakIslemler.getPlanKatSayiAllMap(katsayiList, personelIdler, basTarih, bitTarih, session);
				TreeMap<String, BigDecimal> fmtDurumMap = allMap.containsKey(PuantajKatSayiTipi.GUN_FMT_DURUM) ? allMap.get(PuantajKatSayiTipi.GUN_FMT_DURUM) : null;
				if (fmtDurumMap != null) {
					Long sirketId = aramaSecenekleri.getSirketId(), tesisId = aramaSecenekleri.getTesisId(), vardiyaId = null;
					for (VardiyaGun vg : aylikSablonVardiyalar) {
						String str = vg.getVardiyaDateStr();
						if (ortakIslemler.veriKatSayiVar(fmtDurumMap, sirketId, tesisId, vardiyaId, str)) {
							BigDecimal deger = ortakIslemler.getKatSayiVeriMap(fmtDurumMap, sirketId, tesisId, vardiyaId, str);
							if (deger != null) {
			...
			if (aylikPuantajList != null && userLoginOldu)
				aylikVardiyaOzetOlustur();
		...
		if (list != null && departman != null && departman.isAdminMi()) {
			for (Iterator iterator = list.iterator(); iterator.hasNext();) {
				...
				if (aylikPuantaj.getPersonelDenklestirme().getSuaDurum() != null && aylikPuantaj.getPersonelDenklestirme().getSuaDurum()) {
					...
					continue;
				} else if (aylikPuantaj.getPdksPersonel().getSuaOlabilir() != null && aylikPuantaj.getPdksPersonel().getSuaOlabilir()) {
					aylikPuantaj.setVardiyaSua(Boolean.TRUE);
					continue;
				}
	...
	public ArrayList<Vardiya> fillAylikVardiyaList(AylikPuantaj aylikPuantaj, PersonelDenklestirme pd) {
		...
		try {
			...
			boolean gebeMi = false, sua = false, icap = false, sutIzni = false;
				...
				for (VardiyaGun pdksVardiyaGun : aylikPuantaj2.getVardiyalar()) {
					...
					if (pdksVardiyaGun.isAyinGunu() && pdksVardiya != null) {
						if (pdksVardiya.isCalisma()) {
							...
							if (!sutIzni)
								sutIzni = pdksVardiyaGun.getVardiya().getSutIzni() != null && pdksVardiyaGun.getVardiya().getSutIzni();
						}
			...
			ozelMap.put(Vardiya.SUT_IZNI_KEY, "");
						...
						if (cm.getGenelVardiya().equals(Boolean.FALSE) && cm.isOrtakVardiyadir() == false) {
							...
							if (!list.isEmpty()) {
								...
								for (Vardiya vardiya : list) {
									...
									else if (vardiya.isSutIzniMi())
										key = Vardiya.SUT_IZNI_KEY;
									else
										continue;
	...
	public String vardiyaPlanDosyaYaz() throws Exception {
		...
		for (Iterator iterator = aylikPuantajDosyaList.iterator(); iterator.hasNext();) {
			...
			if (aylikPuantaj.isKaydet() && aylikPuantaj.isSecili()) {
				...
				try {
					PersonelDenklestirme personelDenklestirme = aylikPuantaj.getPersonelDenklestirme();
					if (vardiyaPlanKontrol(aylikPuantaj, personelDenklestirme, calismaModeliVardiyaOzelMap, vardiyaMap, pdksVardiyaPlan, personel.getSicilNo() + " " + personel.getAdSoyad() + " ", true)) {
						// PersonelDenklestirme personelDenklestirme = aylikPuantaj.getPersonelDenklestirme();
						perIdList.add(personel.getId());
						aylikPuantaj.setSecili(Boolean.FALSE);
		...
		if (!islemYapildi)
			...
			suaUyariMesaji(null, aylikPuantajDosyaList);
					...
					if (vardiyaHaftaList != null) {
						...
						for (VardiyaHafta vardiyaHafta : vardiyaHaftaList) {
							...
							if (vardiyaGunler != null) {
								double suaSaat = 0.0d;
								for (VardiyaGun vg : vardiyaGunler) {
									if (vg.getVardiyaDate().before(basTarih) || vg.getVardiyaDate().after(bitTarih))
										continue;
									if (vg.getId() != null && vg.isIzinli() == false) {
										Vardiya vardiya = vg.getVardiya();
										if (vardiya.isCalisma())
	...
	public boolean isTopluGuncelleme() {
		return topluGuncelleme;
	}

	public void setTopluGuncelleme(boolean topluGuncelleme) {
		this.topluGuncelleme = topluGuncelleme;
	}

	public boolean isUserLoginOldu() {
		return userLoginOldu;
	}
