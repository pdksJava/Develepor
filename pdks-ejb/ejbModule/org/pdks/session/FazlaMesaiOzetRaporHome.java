...
public class FazlaMesaiOzetRaporHome extends EntityHome<DepartmanDenklestirmeDonemi> implements Serializable {
	...
	public void fillFazlaMesaiOzetRaporDevam(AylikPuantaj aylikPuantajSablon, DepartmanDenklestirmeDonemi denklestirmeDonemi) throws Exception {
		...
		try {
			...
			if (!personelDenklestirmeler.isEmpty()) {
				...
				Integer tipi1 = sirket.getDepartman().isAdminMi() ? denklestirmeAy.getDenklestirmeTipi() : denklestirmeAy.getTaseronDenklestirmeTipi();
				DenklestirmeTipi denklestirmeTipi = tipi1 != null ? DenklestirmeTipi.fromValue(tipi1) : DenklestirmeTipi.GECEN_AY_ODE;
				...
				for (Iterator iterator1 = puantajDenklestirmeList.iterator(); iterator1.hasNext();) {
					...
					double ucretiOdenenMesaiSure = 0.0d, icapciMesaiSure = 0.0d;
					...
					if (puantaj.getVardiyalar() != null) {
						...
						for (Iterator iterator = puantaj.getVardiyalar().iterator(); iterator.hasNext();) {
							...
							if (vardiyaGun.getResmiTatilToplamSure() != 0.0)
							...
							if (vardiyaGun.getVardiyaSaatDB() != null && vardiyaGun.getDurum()) {
								...
								if (vardiyaSaatDB != null)
									icapciMesaiSure += vardiyaSaatDB.getIcapciMesaiSaat();
								...
								if (fazlaMesaiHesapla) {
									...
									if (vardiyaSaatDB.getResmiTatilToplamSure() > 0.0d) {
										...
										if (vardiyaSaatDB.getResmiTatilKanunenEklenenSure() != null)
											resmiTatilKanunenEklenenSure += vardiyaSaatDB.getResmiTatilKanunenEklenenSure();
						...
						if (resmiTatilKanunenEklenenSureGoster == false)
					...
					for (Iterator iterator = puantaj.getVardiyalar().iterator(); iterator.hasNext();) {
						...
						if (!icapciSaatGoster)
							icapciSaatGoster = icapciMesaiSure > 0.0d;
					...
					if (eksiBakiyeGoster == false) {
						if (puantaj.getEksiBakiyeSuresi() != null && puantaj.getEksiBakiyeSuresi().doubleValue() != 0 && personelDenklestirme.getSonDurum()) {
							eksiBakiyeGoster = true;
						}
					}
				}
				if (resmiTatilVar || haftaTatilVar)
					fazlaMesaiVar = true;
				if (bordroPuantajEkranindaGoster) {
		...
		if (list.isEmpty()) {
			...
			for (AylikPuantaj ap : list) {
				...
				try {
					...
					tablePersonel.addCell(PDFITextUtils.getPdfCell("Adı Soyadı", fontH, Element.ALIGN_CENTER, 2));
					tablePersonel.addCell(PDFITextUtils.getPdfCell(personel.getAdSoyad(), font, Element.ALIGN_CENTER, 4));
					tablePersonel.addCell(PDFITextUtils.getPdfCell(ortakIslemler.personelNoAciklama(), fontH, Element.ALIGN_CENTER));
					tablePersonel.addCell(PDFITextUtils.getPdfCell(personel.getPdksSicilNo(), font, Element.ALIGN_CENTER));
					float[] wd = new float[] { 8, 16, 8, 8, 8, 8, 8, 6, 6, 6, 8, 12 };
					...
					for (VardiyaGun vg : ap.getVardiyalar()) {
						...
						if (personel.isCalisiyorGun(vg.getVardiyaDate())) {
							String aciklama = vardiya.isCalisma() ? authenticatedUser.timeFormatla(vardiya.getBasZaman()) + " - " + authenticatedUser.timeFormatla(vardiya.getBitZaman()) + " [" + vardiya.getKisaAdi() + "]" : vardiya.getKisaAdi();
							table.addCell(PDFITextUtils.getPdfCell(aciklama, font, Element.ALIGN_CENTER));
	...
	private Workbook fazlaMesaiExcelDevam(List<AylikPuantaj> list) {
		...
		if (eksiBakiyeGoster) {
			cell = ExcelUtil.getCell(sheet, row, col++, header);
			ExcelUtil.baslikCell(cell, anchor, helper, drawing, "Devretmeyen Eksi Bakiye", "Sıfırlanıp devretmeyen eksi bakiyeler");
		...
		for (Iterator iter = list.iterator(); iter.hasNext();) {
			...
			try {
				...
				try {
					...
					if (eksiBakiyeGoster) {
						double eksiBakiyeSuresi = 0.0d;
						if (aylikPuantaj.getEksiBakiyeSuresi() != null && aylikPuantaj.getEksiBakiyeSuresi().doubleValue() != 0 && personelDenklestirme.getSonDurum()) {
							eksiBakiyeSuresi = aylikPuantaj.getEksiBakiyeSuresi().doubleValue();
	...
	public Boolean getResmiTatilKanunenEklenenSureGoster() {
