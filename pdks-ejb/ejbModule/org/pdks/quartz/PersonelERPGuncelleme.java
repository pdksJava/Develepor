...
public class PersonelERPGuncelleme implements Serializable {
	...
	private boolean guncellemeDBDurum = Boolean.FALSE, guncellemeHareketDurum = Boolean.FALSE, zamanDurum = Boolean.FALSE, hataGonder = Boolean.FALSE;
	...
	private Session startGuncelleme() {
		...
		try {
			...
			if (sunucuDurum) {
				...
				guncellemeHareketDurum = Boolean.FALSE;
				...
				if (valueHareket != null) {
					guncellemeHareketDurum = PdksUtil.zamanKontrol(PARAMETER_HAREKET_KEY, valueHareket, tarih);
				}
	...
	public void vardiyaHareketGuncelleme(Session session, Date tarih) throws Exception {
		...
		if (denklestirmeList.isEmpty() == false) {
			...
			for (String key : linkedHashMap.keySet()) {
				...
				TreeMap<String, List<PersonelDenklestirme>> pdMap = new TreeMap<String, List<PersonelDenklestirme>>();
				...
				for (PersonelDenklestirme pd : list1) {
					...
					List<PersonelDenklestirme> list2 = pdMap.containsKey(key1) ? pdMap.get(key1) : new ArrayList<PersonelDenklestirme>();
					if (list2.isEmpty())
						pdMap.put(key1, list2);
					list2.add(pd);

				}
				list1 = null;
				for (String dpKkey : pdMap.keySet()) {
					HashMap<Long, PersonelView> kgsPerMap = new HashMap<Long, PersonelView>();
					List<PersonelDenklestirme> list = pdMap.get(dpKkey);
		...
		try {
			...
			if (ortakIslemler.getParameterKeyHasStringValue(ortakIslemler.getParametrePersonelERPTableView())) {
				...
				if (!personelIdList.isEmpty()) {
					...
					bo = ortakIslemler.personelExcelDevam(Boolean.TRUE, personelList, tanimMap, sistemAdminUser, null, false, session);
					if (bo != null) {
